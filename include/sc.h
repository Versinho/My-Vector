...
class vector{
	private:
   	...
   	public:
 		...
 		*/
   		sc::const_iterator<T> cbegin( void )const
   		{ return sc::const_iterator<T>( &data[0] ); }

		/**
		 * returns a constant iterator pointing to the end mark in the list, 
		 i.e. the position just after the last element of the list.
 		*/
   		sc::const_iterator<T> cend( void )const
   		{ return sc::const_iterator<T>( &data[m_size] ); }
 		...
 		*/
   		...
   		{
   			last--; // minha solução para lidar com o intervalo aberto em last.
